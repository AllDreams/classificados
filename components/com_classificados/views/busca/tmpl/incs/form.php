	...
	<div class="control-group">
		...
		<div class="controls">
			<input type="text" id="busca" name="q" required="required" 
				...
				placeholder="<?= JText::_('COM_CLASSIFICADOS_CAMPO_BUSCA') ?>" value="<?= $input->get("q") ?>" />
		</div>
	</div><!--
...
<?php if($ste == null || empty($ste)) : ?>
	<div class="control-group">
		...
		<div class="controls">
			<datalist name="te" id="tipoEmpresa">
				<option></option>
<?php foreach($tiposEmpresa as $item) : ?>
				<option value="<?= $item->id?>"><?=  $item->nome . ' [' . $item->cnae . '] ' ?></option>
<?php endforeach; ?>
			</datalist>
			<input list="tipoEmpresa" name="tipoEmpresa" value="<?= $input->get("tipoEmpresa") ?>" />
		</div>
	</div>
<?php endif; ?>
<?php if($stp == null || empty($stp)) : ?>
	<div class="control-group">
		...
		<div class="controls">
			<datalist name="tp" id="tipoProduto" >
				<option></option>
<?php foreach($tiposProduto as $item) : ?>
				<option value="<?= $item->id?>"><?= $item->nome?></option>
<?php endforeach; ?>
			</datalist>
			<input list="tipoProduto" name="tipoProduto" value="<?= $input->get("tipoProduto") ?>" />
		</div>
	</div>
<?php endif; ?>
